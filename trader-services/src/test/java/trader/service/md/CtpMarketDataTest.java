...
public class CtpMarketDataTest {
    ...
    static Exchangeable SR001 = Exchangeable.fromString("SR001");
    static LocalDate tradingDay = DateUtil.str2localdate("20190902");
    ...
    public void testFromJson() {
        ...
        JsonElement json = ctpTick.toJson();
        MarketData jsonTick = MarketData.fromJson(json);

        assertTrue(jsonTick.producerId.equals(ctpTick.producerId));
        assertTrue(jsonTick.instrument.equals(ctpTick.instrument));
        assertTrue(jsonTick.updateTime.equals(ctpTick.updateTime));
        assertTrue(jsonTick.updateTimestamp==(ctpTick.updateTimestamp));
        assertTrue(jsonTick.volume==ctpTick.volume);
        assertTrue(jsonTick.turnover==ctpTick.turnover);
        assertTrue(jsonTick.averagePrice==ctpTick.averagePrice);
        assertTrue(jsonTick.lastPrice==ctpTick.lastPrice);
        assertTrue(jsonTick.openPrice==ctpTick.openPrice);
        assertTrue(jsonTick.highestPrice==ctpTick.highestPrice);
        assertTrue(jsonTick.lowestPrice==ctpTick.lowestPrice);
        assertTrue(jsonTick.askPrices.length==ctpTick.askPrices.length);
        assertTrue(jsonTick.askPrices[0]==ctpTick.askPrices[0]);
    }
